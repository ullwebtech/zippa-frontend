...
export default function Index() {
  ...
  const benefitTotal = useSelector((state) => state.lock.benefitTotal);
  ...
  return (
    <main className="h-full flex flex-col gap-10">
      ...
      >
        <p className="text-xl font-semibold">What is savings with benefits?</p>
        <p className="">
          This allows you to save fixed amount monthly. You get 100% cashback
      ...
      <div className="flex flex-col md:flex-row justify-between gap-5 px-5 md:p7-10 py-8 w-full rounded-2xl bg-[#E4D9FF]">
        <nav className="flex flex-col gap-3">
          ...
          <b className="font-bold text-primary text-xl">₦{benefitTotal}</b>
          <button
            className="rounded-xl px-5 py-2 text-white bg-primary"
            onClick={() =>
              navigate("/dashboard/savings/save-with-benefit/plans")
            }
          >
            Select a plan
          </button>
      ...
      <div className="flex flex-col gap-5 md:gap-10">
        ...
        <div className="flex flex-col gap-5 md:grid grid-cols-2">
          {benefitSavings?.filter((benefit) => benefit?.status === active).length ===
            0 && (
            <p className="text-center text-primary">
              You have no {active} benefit savings
            </p>
          )}
          {benefitSavings
...
const Single = ({ title, amount, endDate, startDate, slug }) => {
  ...
  return (
    <div
      ...
      onClick={() =>
        navigate(`/dashboard/savings/save-with-benefit/${slug}`)
      }
    >
