...
import {
  createBenefitLock,
  createBenefitLockCard,
  createBenefitLockNewCard,
...
export const CreateBenefit = ({ setShowModal, nextStep, plan, amount }) => {
  ...
  function handleNewCard() {
    ...
    paystack.newTransaction({
      ...
      onSuccess: (transaction) => {
        ...
        dispatch(createBenefitLockNewCard(formDetails));
      },
      onCancel: () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (source === "card") {
      ...
      dispatch(createBenefitLockCard(formDetails));
    }
    if (source === "wallet") {
      const formDetails = new FormData();
      formDetails.append("plan", plan);
      formDetails.append("startDate", startDate);
      formDetails.append("dayOfMonth", dayOfMonth);
      dispatch(createBenefitLock(formDetails));
    }
  };

  useEffect(() => {
    if (message === "Benefit Lock Created Successfully") {
  ...
  return (
    <div className="w-full h-full relative p-5 md:p-10 flex flex-col justify-center gap-5 md:gap-8">
      ...
      <h3 className="text-xl font-medium">
        Savings with benefits - <b className="uppercase">{plan}</b>
      </h3>
      <CustomInput
        name="Start Date"
        value={startDate}
      ...
      <div className="flex flex-col gap-2">
        ...
        <label className="flex items-center gap-2">
          ...
          />{" "}
          I authorize Zippa to deduct {amount} monthly from my wallet
        </label>
      </div>
